...
BEGIN
  ...
  FROM transactions 
  WHERE id = p_transaction_id AND user_id = p_user_id AND type = 'p2p_ariary';

  IF v_transaction_data IS NULL THEN
    RETURN json_build_object('success', false, 'error', 'Transaction P2P Ariary not found');
  END IF;

  -- Calculate fee: amount_ariary -> AXE
  -- 1 AXE = 1000 Ariary
  v_fee_amount := (v_transaction_data.montant / 1000.0) * (p_fee_percentage / 100.0);
  ...
  BEGIN
    ...
    VALUES (p_user_id, 'p2p_ariary', v_fee_amount);
